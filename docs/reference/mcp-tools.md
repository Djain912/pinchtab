...
| --- | --- | --- |
| `pinchtab_navigate` | `url` required, `tabId`, `newTab`, `snap` | Uses `/navigate`; reuses the current tab, and `newTab=true` opens one (as does a first navigate with no tab yet). The returned `tabId` targets the new tab in later tools. `snap=true` returns an interactive compact snapshot in the same response |
