...
func allTools() []mcp.Tool {
	return []mcp.Tool{
		...
		mcp.NewTool("pinchtab_click",
			...
			stringAliasParam("onDialog", "dialogAction"),
			stringAliasParam("promptText", "dialogText"),
		...
		mcp.NewTool("pinchtab_handoff",
			mcp.WithDescription("Pause a tab for human handoff (a CAPTCHA, a login, a consent wall the agent must not solve). While paused every action tool on that tab is refused with tab_paused_handoff. Return control to the user, ask them to finish in the browser, then call pinchtab_resume. Other tabs are unaffected."),
			requiredTabIDParam(),
			mcp.WithString("reason", mcp.Description("Why the tab is handed off, e.g. captcha_manual or login_required (default manual_handoff)")),
			mcp.WithNumber("timeoutMs", mcp.Description("Optional auto-resume after this many milliseconds; 0 or omitted means the tab stays paused until pinchtab_resume")),
		),
		mcp.NewTool("pinchtab_resume",
			mcp.WithDescription("Resume a tab paused for human handoff so action tools work again. Call it only after the user has confirmed they finished the manual step; check with pinchtab_handoff_status first when unsure."),
			requiredTabIDParam(),
			mcp.WithString("status", mcp.Description("Optional note recorded with the resume, e.g. completed or failed")),
		),
		mcp.NewTool("pinchtab_handoff_status",
			mcp.WithDescription("Report a tab's handoff state: status paused_handoff with reason, pausedAt and any expiresAt while a human holds it, or status active once resumed or auto-resumed by its timeout."),
			requiredTabIDParam(),
		),
		...
		mcp.NewTool("pinchtab_wait",
			...
			timeoutMsParam(),
			numberAliasParam("timeout", "timeoutMs"),
		...
		mcp.NewTool("pinchtab_scrape",
			...
			mcp.WithNumber("timeoutSeconds", mcp.Description("Overall HTTP crawl budget in SECONDS (default 60); unlike the millisecond timeoutMs of pinchtab_wait and pinchtab_handoff")),
			browserParam(),
		),
		a11yAuditTool(),
		memoryTool(),
